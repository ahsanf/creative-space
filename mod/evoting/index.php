...
$PAGE -> set_url('/mod/evoting/index.php', array('id' => $id));
...
if (!$evotings = get_all_instances_in_course('evoting', $course)) {
	notice(get_string('noevotings', 'evoting'), new moodle_url('/course/view.php', array('id' => $course -> id)));
}
...
$currentsection = '';
foreach ($modinfo->instances['evoting'] as $cm) {
	$row = array();
	if ($usesections) {
